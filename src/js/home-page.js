...
const getLessons = (chapterData) =>
  sortBy([...chapterData].filter((chapter) => !chapter.hidden), (chapter) =>
    ...
    return (
      <div className="homepage">
        ...
        */}
        ...
        <div
          className={`homepage-banner-area ${currentGrade ? "open" : ""} ${
            showLiveClassButton ? "liveClassButtonIsVisible" : ""
          }`}
        >
          <div className="homepage-title-primary">
            <img src={logo} className="homepage-logo" />
          </div>
          <div className="homepage-title-secondary">Learn new things in 40 min or less</div>
          ...
          {showLiveClassButton && (
            <a
              target="_blank"
              href="https://meet.google.com/udp-uepj-vru"
              className={
                "live-banner " +
                (this.state.grade6 || this.state.grade7 ? "live-banner-invisible" : "")
              }
            >
        ...
        <div
          className={`homepage-grade-section ${
            currentGrade ? "homepage-grade-section-visible" : "homepage-grade-section-invisible"
          }`}
        >
          {currentGrade && (
            <div className="container homepage-grade-section-body">
              <div
                className={`pure-g ${currentGrade ? "selected-chapter" : "not-selected-chapter"}`}
              >
